			...
			<div class="card-body">
				<div class="easyui-layout">
					...
					<table id="dgGrid"
						...
						rowNumbers="true"
						pagination="true"
						url="<?= base_url('admin/get_dr_lv3/' . urlencode($lv2_id) . '/' . urlencode($table1) . '/' . urlencode($table2)) ?>"
						pageSize="10"
						...
						<thead>
							<tr>
								<th field="lv3_id" hidden="yes" width="30%">id</th>
			...
			<div class="card-body">
				<div class="easyui-layout">
					<table id="dgGrid_file" title="File"
						...
						rowNumbers="true"
						pagination="true"
						url="<?= base_url('admin/get_dr_lv3_file/' . urlencode($lv2_id) . '/' . urlencode($table1) . '/' . urlencode($table2)) ?>"
						pageSize="10"
						pageList="[5,10,20,50,75,100,125,150,200]"
						nowrap="true"
						singleSelect="false"
...
<script type="text/javascript">
	...
	function editForm() {
		...
		if (row) {
			...
			url = '<?= base_url("admin/updatenamedirectory") ?>?id=' + row.lv3_id + '&table=sd_ho_lv3';
	...
	function formatLink(value, row, index) {
		...
		} else if (table1 === "sd_ho_lv2") {
			let newTable1 = "sd_ho_lv3";
			let newTable2 = "sd_ho_lv4";
			// Ganti '/' dengan karakter lain, misalnya '-'
			let safeName = row.name.replace(/\//g, '-');
			return '<a href="<?= base_url('admin/dr_lv4/') ?>' + row.lv3_id + '/' + newTable1 + '/' + newTable2 + '/' + encodeURIComponent(safeName) + '">Open</a>';
		}
	}
