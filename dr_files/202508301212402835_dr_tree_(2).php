			...
			<div class="card-body">
				<div class="easyui-layout">
					<div class="row" style="margin-bottom: 10px;">
						<div class="col-md-6">
							<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-tip" plain="false" onclick="toggleTable()">Show/Hide Directory</a>
							<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-add" plain="false" onclick="newForm()">New Directory</a>
							<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="false" onclick="editForm()">Rename</a>
							<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-redo" plain="false" onclick="moveDirectory()">Move Directory</a>
					...
					<table id="dgGrid"
						...
						rowNumbers="true"
						pagination="true"
						url="<?= base_url('admin/get_dr_tree/' . urlencode($lv1_id)) ?>"
						pageSize="100"
						...
						<thead>
							<tr>
								<th field="id" hidden="yes" width="30%">id</th>
			...
			<div class="card-body">
				<div class="easyui-layout">
					<table id="dgGrid_file" title="File"
						...
						rowNumbers="true"
						pagination="true"
						url="<?= base_url('admin/get_dr_file_tree/' . urlencode($lv1_id)) ?>"
						pageSize="10"
						pageList="[5,10,20,50,75,100,125,150,200]"
						nowrap="true"
						singleSelect="false"
					...
					<div id="toolbar_file" style="padding: 10px">
						<div class="row ml-1">
							<div class="col-sm-6">
								<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-add" plain="false" onclick="newFile()">Upload File</a>
								...
								<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-redo" plain="false" onclick="openMoveFileDialog()">Move File</a>
...
<script type="text/javascript">
	...
	function newForm() {
		...
		$('#id_parent').val("<?= isset($lv1_id) ? $lv1_id : '' ?>");
	...
	function destroy() {
		var selectedRow = $('#dgGrid').datagrid('getSelected');
		if (selectedRow) {
			$.messager.confirm('Confirm', 'Delete "' + selectedRow.name + '"?', function(r) {
				if (r) {
					$.ajax({
						...
						type: 'POST',
						data: {
							id: selectedRow.id // pastikan ini adalah nama kolom ID utama
						},
						dataType: 'json',
						success: function(response) {
							if (response.success) {
	...
	function formatLink(value, row, index) {
		return '<a href="<?= base_url('admin/dr_tree/') ?>' + row.id + '">Open</a>';
	}
	...
	function newFile() {
		...
		$('#ff').form('clear');
		$('#parent_id').val("<?= isset($lv1_id) ? $lv1_id : '' ?>");
		$('#id_user').val(idUser);
	...
	function submitMultiUpload() {
		...
		$('#parent_id_multi').val("<?= isset($lv1_id) ? $lv1_id : '' ?>"); // Pastikan 'id' didefinisikan di scope global atau passed sebagai argumen
	...
	function moveDirectory() {
		...
		if (!row) {
			Toast.fire({
				icon: 'error',
				title: 'Please select a directory to move.'
			});
			return;
		}
