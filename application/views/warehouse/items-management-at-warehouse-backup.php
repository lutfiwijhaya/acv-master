...
<div class="col-12">
	<div class="card">
			...
			<div class="card-body">
				<div class="easyui-layout">
					...
					<div id="toolbar" style="padding: 10px">
						<div class="row ml-1">
							<div class="col-sm-6">
								<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-add" plain="false" onclick="newForm()">In</a>
								<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="false" onclick="editForm()">Out</a>
								<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-print" plain="false" onclick="delete_item()">Request</a>
								<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-reload" plain="false" onclick="adddist()">Request Status</a> <span style="margin-left: 10px; vertical-align: middle;">
		...
		<div id="dialog-form-dist" class="easyui-window" title="Add New Distribution" data-options="modal:true,closed:true,iconCls:'icon-save',inline:false,onResize:function(){
    						$(this).window('hcenter');}" style="width:100%;max-width:500px;padding:30px 60px;max-height:500px;overflow-y:auto;">
			<form id="ff-dist" class="easyui-form" method="post" data-options="novalidate:false" enctype="multipart/form-data">
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="tanggal" name="tanggal" type="date" style="width:100%" data-options="label:'Tanggal:',required:true">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="no_req-dist" name="no_req" style="width:100%"
						data-options="label:'Request Number:',required:false,valueField:'param_name',textField:'param_name'">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-combobox" id="dist_type-dist" name="dist_type" style="width:100%"
						data-options="label:'Distribution type:',required:true,valueField:'param_name',textField:'param_name'">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="category-dist" editable="false" name="category" style="width:100%"
						data-options="label:'Category:',required:true,valueField:'param_name',textField:'param_name',url:''">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="kode_barang-dist" editable="false" name="kode_barang" style="width:100%"
						data-options="label:'Kode_barang:',required:true,valueField:'kode_barang',textField:'kode_barang',url:''">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="inisial_kuantitas-dist" editable="false" name="inisial_kuantitas" style="width:100%" data-options="label:'Quantity Unit:',required:false">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="level_1-dist" editable="false" name="level_1" style="width:100%" data-options="label:'level_1:',required:false">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="level_2-dist" editable="false" name="level_2" style="width:100%" data-options="label:'level_2:',required:false">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="level_3-dist" editable="false" name="level_3" style="width:100%" data-options="label:'level_3:',required:false">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="level_4-dist" editable="false" name="level_4" style="width:100%" data-options="label:'level_4:',required:false">
				</div>
				<div style="margin-bottom:20px">
				...
				<div style="margin-bottom:20px">
					<input class="easyui-combobox" id="from_name-dist" name="from_name" style="width:100%"
						data-options="label:'Name:',required:true,valueField:'wh_name',textField:'wh_name',url:''">
				</div>
				<div style="margin-bottom:20px">
					<input class="easyui-textbox" id="stok-dist" editable="false" name="stok" style="width:100%" data-options="label:'stok:',required:false">
				</div>
				...
				<div style="margin-bottom:20px">
					<input class="easyui-numberbox" id="quantity-dist" editable="true" name="quantity" style="width:100%" data-options="label:'quantity:',required:false">
				</div>
				<div style="margin-bottom:20px">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		// 		var category = $('#category-dist').combobox('getValue');
		...
		// 					$('#category-dist').textbox('setValue', data.category);
		// 					$('#inisial_kuantitas-dist').textbox('setValue', data.inisial_kuantitas);
		// 					$('#level_1-dist').textbox('setValue', data.level_1);
		// 					$('#level_2-dist').textbox('setValue', data.level_2);
		// 					$('#level_3-dist').textbox('setValue', data.level_3);
		// 					$('#level_4-dist').textbox('setValue', data.level_4);
	...
	function submitFormDist() {
		// Ambil nilai dari elemen input
		var quantity = $('#quantity-dist').numberbox('getValue');
		...
		$('#ff-dist').append('<input type="hidden" name="id_from" value="' + id_from + '">');
		$('#ff-dist').append('<input type="hidden" name="from" value="' + from + '">');
		$('#ff-dist').append('<input type="hidden" name="item_id" value="' + item_id + '">');
		$('#ff-dist').append('<input type="hidden" name="id_to" value="' + id_to + '">');
		$('#ff-dist').append('<input type="hidden" name="to" value="' + to + '">');

		// Serialisasi form setelah semua field ditambahkan
		var string = $("#ff-dist").serialize();
		...
		$('#ff-dist').form('submit', {
			...
			success: function(result) {
				...
				if (result.errorMsg) {
					...
					$('#dialog-form-dist').dialog('close'); // Close the dialog
	...
	function adddist() {
		...
		if (row) {
			...
			id_to = '';
			to = '';
			$('#dialog-form-dist').dialog('open').dialog('setTitle', 'Distribute Item');
			$('#ff-dist').form('clear');
			$('#category-dist').textbox('setValue', row.category);
			$('#kode_barang-dist').textbox('setValue', row.kode_barang);
			$('#inisial_kuantitas-dist').textbox('setValue', row.inisial_kuantitas);
			$('#level_1-dist').textbox('setValue', row.Level_1);
			$('#level_2-dist').textbox('setValue', row.level_2);
			$('#level_3-dist').textbox('setValue', row.level_3);
			$('#level_4-dist').textbox('setValue', row.level_4);
			item_id = row.id;
	...
	$('#from-dist').combobox({
		onSelect: function(record) {
			...
			if (remark == 'tbl_user') {
				$('#from_name-dist').combobox('options').valueField = 'nama';
				$('#from_name-dist').combobox('options').textField = 'nama';
				$('#from_name-dist').combobox('reload', '<?= base_url('admin/getFromId') ?>?remark=' + remark);
				from = 'employee_id';

			} else if (remark == 'wh_warehouse') {
				$('#from_name-dist').combobox('options').valueField = 'wh_name';
				$('#from_name-dist').combobox('options').textField = 'wh_name';
				$('#from_name-dist').combobox('reload', '<?= base_url('admin/getFromId') ?>?remark=' + remark);
				from = 'warehouse_id';
	...
	$('#to-dist').combobox({
		onSelect: function(record) {
			var remark = record.remark; // Ambil nilai remark yang dipilih
			if (remark == 'tbl_user') {
				$('#to_name-dist').combobox('options').valueField = 'nama';
				$('#to_name-dist').combobox('options').textField = 'nama';
				$('#to_name-dist').combobox('reload', '<?= base_url('admin/getFromId') ?>?remark=' + remark);
				to = 'employee_id';
			} else if (remark == 'wh_warehouse') {
				$('#to_name-dist').combobox('options').valueField = 'wh_name';
				$('#to_name-dist').combobox('options').textField = 'wh_name';
				$('#to_name-dist').combobox('reload', '<?= base_url('admin/getFromId') ?>?remark=' + remark);
				to = 'warehouse_id';
			} else if (remark == 'tbl_supplier') {
				$('#to_name-dist').combobox('options').valueField = 'nama';
	...
	function checkStock(item_id) {
		...
		$.getJSON('<?= base_url('admin/checkStock') ?>', {
			...
			if (data && data.quantity) {
				...
				$('#stok-dist').textbox('setValue', data.quantity);
			} else {
				// Jika stok tidak ditemukan, tampilkan pesan dan reset field 'Stok'
				// alert('Stock not found');
				$('#stok-dist').textbox('setValue', '0'); // Mengatur nilai ke 0 jika stok tidak ditemukan
			}
		});
	}
